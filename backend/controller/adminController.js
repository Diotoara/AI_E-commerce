...
export const dashboardStats = catchAsyncError(async(req,res,next)=> {
    ...
    const topSellingProductsQuery = await database.query(`
        ...
        GROUP BY p.name, images, p.category, p.ratings
    ...
    const topSellingProducts = topSellingProductsQuery.rows;
        ...
        //totall sales of currecnt month
    ...
    //producrs with stock less than or equal to 5
    const lowStockProductsQuery = await database.query(`
        SELECT name, stock FROM products WHERE stock <= 5
    `);

    const lowStosckProducts = lowStockProductsQuery.rows;
    ...
    const lastMonthRevnue = parseFloat(lastMonthRevenueQuery.rows[0].total) || 0;
    let revenueGrowth = "0%";
    if(lastMonthRevnue > 0){
        const growthRate = ((currentMonthSales - lastMonthRevnue) / lastMonthRevnue) * 100;
    ...
    const newUsersThisMonthQuery = await database.query(`
        ...
        WHERE created_at BETWEEN $1 AND $2 AND role = 'User'
    `,
    [currentMonthStart, currentMonthEnd]
    );
